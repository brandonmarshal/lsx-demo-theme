...
-   Read-only research (search, fetch, analysis) does not require explicit user permission.
-   Write operations (code creation, file edits, command execution) require explicit user permission before each action.

### 0. Investigation Planning

**Spike Document Validation Gate:**

-   At the start of this phase, check that a spike document path argument is provided.
-   Verify the file exists and is readable, or that the path is syntactically valid for remote documentation.
-   If the check fails, emit a clear error message: "ERROR: No valid spike document provided. Halting research protocol."
-   Stop execution and do not proceed to subsequent phases (return non-zero exit/throw/stop further steps).
...
**Obsessive Documentation Mining**: Research every angle exhaustively (read-only; no explicit permission required)
...
**Recursive Code Investigation**: Follow every implementation trail (read-only; no explicit permission required)
...
**ASK USER PERMISSION before any code creation or command execution (write operations)**
...
-   Create test files using `#edit` tools (explicit permission required)
-   Execute validation using `#runCommands` or `#runTasks` tools (explicit permission required)
